...
class RENet(nn.Module):
    ...
    def cca(self, spt, qry):
        ...
        num_qry, num_spt, H_s, W_s, H_q, W_q = corr4d.size()

        # corr4d refinement
        corr4d = self.cca_module(corr4d.view(-1, 1, H_s, W_s, H_q, W_q))
        corr4d_s = corr4d.view(num_qry, num_spt, H_s * W_s, H_q, W_q)
        corr4d_q = corr4d.view(num_qry, num_spt, H_s, W_s, H_q * W_q)
        ...
        corr4d_s = corr4d_s.view(num_qry, num_spt, H_s, W_s, H_q, W_q)
        corr4d_q = F.softmax(corr4d_q / self.args.temperature_attn, dim=4)
        corr4d_q = corr4d_q.view(num_qry, num_spt, H_s, W_s, H_q, W_q)
        ...
        if self.args.shot > 1:
            ...
            support_weights = self.compute_support_weights(spt_for_weight)
            support_weights = support_weights.permute(1, 0).unsqueeze(0)
            for _ in range(spt_attended.dim() - 3):
                support_weights = support_weights.unsqueeze(-1)
            spt_attended = (spt_attended * support_weights).sum(dim=1)
    ...
    def compute_support_weights(self, spt_feats, temperature=0.8):
        shot = self.args.shot
        way = self.args.way

        if shot <= 1:
            return spt_feats.new_ones(way, shot)

        b, c, h, w = spt_feats.shape
        spt_feats = spt_feats.view(way, shot, c, h, w)
        spt_feats = spt_feats.mean(dim=[-1, -2])
        spt_feats = F.normalize(spt_feats, p=2, dim=-1, eps=1e-8)

        sim = torch.matmul(spt_feats, spt_feats.transpose(-1, -2))
        eye = torch.eye(shot, device=sim.device, dtype=sim.dtype).unsqueeze(0)
        # sim = sim - eye
        # sim = sim.clamp_min(0.0)
        # weights = sim.sum(dim=-1) / (shot - 1 + 1e-8)
        # weights = weights / (weights.sum(dim=-1, keepdim=True) + 1e-8)
        # return weights
        mask = 1 - eye  # 非对角线元素为1，对角线为0
    
        # 应用mask并计算每个shot与其他shot的平均相似度
        masked_sim = sim * mask
        avg_similarity = masked_sim.sum(dim=-1) / (shot - 1 + 1e-8)  # [way, shot]
        
        # 使用softmax计算权重，温度参数控制分布尖锐程度
        weights = F.softmax(avg_similarity / temperature, dim=-1)
